...
double kp= 2,kd = 0;
double kp_d= 0.5,kd_d = 0.1;
long int t = millis(),stop_time=0;
// ================================================================
// ===               Variables for Tf mini Lidar                ===
// ================================================================
double dist1,dist2; //actual distance measurements of LiDAR 
...
float cap1,cap2,cap3,cap4, x, y,slosh_lim = 1.5, safe_slosh_lim = .75;
...
float d_mag[4];
RunningMedian a_x = RunningMedian(15);
RunningMedian a_y = RunningMedian(15);
...
float acc_x_reading, acc_y_reading, acc_x_offset, acc_y_offset,  acc_z_reading, acc_z_offset;
volatile bool mpuInterrupt = false;     // indicates whether MPU interrupt pin has gone high
float vel_x,vel_y,vel_mag;
// ================================================================
// ===                      INITIAL SETUP                       ===
// ================================================================
void setup() {
...
//    slosh_xy();
    xbox_control();
    if(flag_stop&&((millis()-stop_time)<5000)){
      pwm_front_right = 127;
      pwm_front_left = 127;
      pwm_back_right = 127;
      pwm_back_left = 127;
...
//  roll_reading = (ypr[2] - roll_offset) * 180 / M_PI;
  a_x.add(accl[0] - acc_x_offset);
  a_y.add(accl[1] - acc_y_offset);
  acc_z_reading = accl[2] - acc_z_offset;// - acc_z_offset;
  acc_x_reading = a_x.getMedian();
  acc_y_reading = a_y.getMedian();
  print_gyro();
  cal_velocity();
  print_vel();
  print_acc();
