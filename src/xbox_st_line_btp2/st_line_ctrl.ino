void move_straight(double target_angle){
  ...
  static double orientation_error, prev_error, d_orientation_error,control_output=0, act_dist2;
  act_dist2 = dist2*cos(yaw_reading*PI/180);
  if(flag_auto)Serial.println(act_dist2);
  if(act_dist2<60){
    if(dir == 1){
    pwm_front_right = 127;
    pwm_front_left = 127;
