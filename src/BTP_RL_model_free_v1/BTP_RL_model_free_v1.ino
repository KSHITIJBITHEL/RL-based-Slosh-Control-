...
//      v_rl = 2;
      ...
      t_rl = millis();
      reward = -(2 * abs(act_disty - target) +5*abs(velocity)+ 0.3 * abs(phi) + 0.3 * abs(phi_dot));
      if(flag_stop) reward -= 3000;
      if(abs(act_disty-target)<100) reward += 500;
      if(abs(act_disty-target)<100 && abs(velocity)<2) reward += 1000;
