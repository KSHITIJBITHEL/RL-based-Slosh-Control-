void move_straight(double target_angle){
  ...
  static double orientation_error, prev_error, d_orientation_error,control_output=0, dir= 1, prev_dir=-1,gamma = 0.03;
...
//  if(flag_auto)Serial.print(act_disty);
  ...
  if(incr) base_pwm = base_pwm+vel_fuzzy(des_vel)/3;
  else base_pwm = 0;
  base_pwm = int(base_pwm);
//  Serial.print(dir);
  orientation_error = target_angle - yaw_reading ;
  d_orientation_error = orientation_error - prev_error ; 
   kp -= gamma * orientation_error *orientation_error ;
  kd -= gamma * orientation_error * d_orientation_error;
//  ki -= gamma * vel_error * i_vel_error;
