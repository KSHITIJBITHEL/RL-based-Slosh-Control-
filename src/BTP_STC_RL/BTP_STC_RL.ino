...
double c2 = 7,k1 = 1.5,k2 = 4,phi,phi_dot=0,reward=0,des_v=0,vel_integral =0;
String s,cs2,ks1,ks2;
bool flag_go_back =0;
// ================================================================
...
while(Serial.available()){
  ...
  while(Serial.available()) s+= char(Serial.read());
    cs2 = s.substring(0,s.indexOf('o'));
    s =  s.substring(s.indexOf('o')+1,s.length());
    ks1 = s.substring(0,s.indexOf('o'));
    s =  s.substring(s.indexOf('o')+1,s.length());
    ks2 = s.substring(0,s.indexOf('o'));
    c2 = cs2.toFloat();
    k1 = ks1.toFloat();
    k2 = ks2.toFloat();
    t_one_run = millis();
...
//        i_vel_error = 0;
  ...
  //  print_pwm();
  //  print_gyro();
    reward += -(0.004*abs(act_disty)+0.3*abs(phi)+0.3*abs(phi_dot));
    send_motor_pwm();
  //  delay(12);
}
